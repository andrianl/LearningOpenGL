...
{
public:
    // Get the instance of the GlobalVAO (Singleton)
    // This static method returns a reference to the single instance of GlobalVAO.
    static GlobalVAO& GetInstance()
    {
        static GlobalVAO instance;  // Guaranteed to be destroyed and instantiated on first use.
        return instance;
    }

    // Initialize the Global VAO
    // This method initializes the Global VAO by generating a unique ID for it using OpenGL.
    void Initialize() { glGenVertexArrays(1, &ArrayID); }

    // Delete the Global VAO
    // This method deletes the Global VAO by its unique ID to free up resources.
    void Delete() const { glDeleteVertexArrays(1, &ArrayID); }

    // Bind the Global VAO
    // This method binds the Global VAO, making it the current VAO to which OpenGL commands will apply.
    ...
    // This method binds a Vertex Buffer Object (VBO) to the Global VAO, specifying it as the current buffer for vertex attributes.
    //
    // Parameters:
    // - vboID: The unique ID of the VBO to attach.
    ...
    // This method binds an Element Buffer Object (EBO) to the Global VAO, specifying it as the current buffer for index data.
    //
    // Parameters:
    // - eboID: The unique ID of the EBO to attach.
    ...
    // Get the array ID
    // This method returns the unique ID of the Global VAO.
    //
    // Returns:
    // - The unique ID of the Global VAO as a GLuint.
    GLuint GetArrayID() const { return ArrayID; }

private:
    GLuint ArrayID;  // The unique ID of the Global VAO
